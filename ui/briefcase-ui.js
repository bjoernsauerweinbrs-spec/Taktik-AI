window.BriefcaseUI = {
    ...
    renderSporttasche: function() {
        ...
        document.getElementById('active-content').innerHTML = '<div style="padding: 10px;">' +
            ...
            players.map(function(p) {
                return '<div class="p-card" onclick="BriefcaseUI.openFIFAcard(\'' + p.id + '\')" style="background: #151515; border: 1px solid #333; padding: 15px; border-radius: 10px; text-align: center; cursor:pointer;">' +
        ...
        document.getElementById('active-content').innerHTML = `
            <div style="display: flex; gap: 30px; background: #000; padding: 25px; border-radius: 15px; border: 1px solid #ff9500;">
                <div id="card-preview" style="width: 220px; height: 320px; background: linear-gradient(145deg, #d4af37, #b8860b); border-radius: 10px; padding: 20px; color: #111; text-align: center;">
                    ...
                    <div style="font-size: 1.5rem; font-weight: 900; margin-top: 60px; text-transform: uppercase;">${p.name}</div>
                    <div style="display: grid; grid-template-columns: 1fr 1fr; margin-top: 20px; font-weight: bold; font-size: 0.8rem; text-align: left;">
                ...
                <div style="flex-grow: 1;">
                    ...
                    <button class="login-btn" style="width:100%; margin-top:15px;" onclick="BriefcaseUI.renderSporttasche()">FERTIG</button>
                </div>
            </div>`;
    },
    ...
    sync: function(id) {
        ...
        var rating = Math.round((parseInt(vals.pace) + parseInt(vals.shooting) + parseInt(vals.passing) + parseInt(vals.ginga) + parseInt(vals.defense) + parseInt(vals.stamina)) / 6);
    ...
    renderSystem: function() {
        ...
        document.getElementById('active-content').innerHTML = '<div style="padding: 20px;">' +
            '<h4 style="color:#fff;">KI-SETUP (API-KEY)</h4>' +
            '<input type="password" id="api-key-input" class="login-input" style="width:100%; background:#000; color:#fff; border:1px solid #444; padding:10px;" value="' + currentKey + '">' +
    ...
    renderPlaceholder: function(s) { document.getElementById('active-content').innerHTML = '<div style="padding:50px; color:#555;">Sektor ' + s + ' folgt.</div>'; },
