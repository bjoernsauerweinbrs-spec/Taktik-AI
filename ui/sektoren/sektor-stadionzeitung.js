 ...
 * Fokus: A4-Druck, Automatischer Taktik-Sync & Redaktion
 * Status: CLEAN & PRINT-READY 2026
 */
window.SektorStadionzeitung = {
    ...
    open() {
        ...
        // Sicherstellen, dass das Magazin in der Datenbank existiert
        if (window.Database && !window.Database.magazine) {
    ...
    render() {
        const content = document.getElementById('active-content');
        const magazine = window.Database.magazine || { pages: [] };
                ...
                <div style="overflow-y: auto; padding: 25px; background: rgba(255,255,255,0.02); border-radius: 15px; border: 1px solid rgba(57, 255, 20, 0.1);">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:25px;">
                        <div>
                            ...
                            <span style="color:#666; font-size:0.6rem;">A4 PRINT ENGINE AKTIV // V. 2.0</span>
                        </div>
                ...
                <div style="background: rgba(0,0,0,0.3); border-left: 1px solid #222; padding: 25px; display:flex; flex-direction:column; border-radius:15px;">
                    ...
                    <div style="flex:1; overflow-y:auto;">
                        ...
                        ${magazine.pages.map((p, idx) => `
                            <div style="background:#05080F; padding:15px; border-radius:10px; margin-bottom:10px; display:flex; justify-content:space-between; align-items:center; border:1px solid #222;">
                                <div style="display:flex; flex-direction:column;">
                                    <span style="font-size:0.75rem; color:#fff; font-family:'Orbitron';">S.${idx+1} ${p.type.toUpperCase()}</span>
                                    <span style="font-size:0.5rem; color:#444;">${(p.title || p.headline || 'Inhalt').substring(0, 15)}...</span>
                                </div>
    ...
    renderPageHTML(p, idx) {
        ...
        if (p.type === 'lineup') {
            return `
                <div class="print-page" style="${baseStyle}">
                    ...
                    <div style="width:100%; height:420px; background:#0a150a; border-radius:10px; border:2px solid #000; position:relative; display:flex; justify-content:center; align-items:center;">
                        <div style="color:#39FF14; font-family:'Orbitron'; font-size:0.8rem; text-align:center;">
                            ...
                            TAKTIK-DATEN SYNC AKTIV
                        </div>
                    </div>
                    <div style="margin-top:30px; padding:20px; background:#f9f9f9; border-left:5px solid #000;">
                        <h4 style="margin:0 0 10px 0; font-family:'Orbitron';">TONI'S TIPP:</h4>
                        <p style="font-style:italic; font-size:0.95rem; line-height:1.6;">"Basierend auf den Biometrie-Daten im Labor empfehle ich für ${club} heute eine hohe Intensität in den ersten 20 Minuten."</p>
                    </div>
                </div>`;
        }
        ...
        return `
            <div class="print-page" style="${baseStyle}">
                ...
                ${p.img ? `<img src="${p.img}" style="width:100%; border-radius:5px; margin-bottom:25px; max-height:250px; object-fit:cover;">` : ''}
    ...
    openEditor(idx) {
        ...
        let inputs = `<label style="font-size:0.5rem; color:#666;">TYP: ${p.type.toUpperCase()}</label><br><br>`;
        
        if (p.type === 'cover') {
            inputs += `
                <label>TITEL</label><input type="text" id="edit-mag-title" value="${p.title}" style="width:100%; background:#000; color:#fff; border:1px solid #333; padding:12px; margin-bottom:20px; font-family:'Orbitron';">
                ...
                <label>BILD URL</label><input type="text" id="edit-mag-img" value="${p.img}" style="width:100%; background:#000; color:#fff; border:1px solid #333; padding:12px; margin-bottom:20px;">
            `;
        } else {
            inputs += `
                ...
                <label>BILD URL</label><input type="text" id="edit-mag-img" value="${p.img || ''}" style="width:100%; background:#000; color:#fff; border:1px solid #333; padding:12px; margin-bottom:20px;">
                <label>TEXT</label><textarea id="edit-mag-text" style="width:100%; height:180px; background:#000; color:#fff; border:1px solid #333; padding:12px; margin-bottom:20px; font-family:sans-serif;">${p.text || ''}</textarea>
            `;
        }

        content.innerHTML = `
            <h3 style="color:var(--neon-green); font-size:0.9rem; margin-bottom:25px;">REDAKTION S.${idx+1}</h3>
            ${inputs}
    ...
    addPage() {
        window.Database.magazine.pages.push({ type: 'article', headline: 'NEUE STORY', text: 'Hier Text...', img: '' });
        window.Database.save();
        this.render();
    },

    removePage(idx) {
        if (idx === 0) return alert("Cover ist fest!");
    ...
    triggerPrint() {
        ...
        printWindow.document.write(`
            <html>
                <head>
                    <title>TONI 2.0 // PRINT ENGINE</title>
