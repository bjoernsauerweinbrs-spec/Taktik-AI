 ...
 * Fokus: Biometrie-Sync, FIFA-Card-Logik & Echtzeit-Diagnose-Editor
 * Status: MASTER-SYNC 2026 - FULL CONTROL
 */
                ...
                <div style="background: rgba(0,0,0,0.2); border-right: 1px solid rgba(255,255,255,0.05); padding-right: 15px; overflow-y: auto;">
                    ...
                    ${players.map(p => {
                        ...
                        <div onclick="window.SektorAnalyse.selectPlayer('${p.id}')" 
                            ...
                            <div style="font-size: 0.55rem; color: #666;">BMI: ${this.calculateBMI(p.height, p.weight)}</div>
                        </div>`;
                    }).join('')}
                </div>
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 30px; border-bottom: 1px solid #333; padding-bottom: 15px;">
                        <div>
                            ...
                            <div style="display:flex; gap:15px; margin-top:8px;">
                                ...
                                <button onclick="window.SektorAnalyse.openEditor()" style="background:none; border:none; color:var(--data-cyan); cursor:pointer; font-size:0.6rem; font-family:'Orbitron';">[ WERTE ANPASSEN ]</button>
                            </div>
                        </div>
                        <div style="background: ${cardStyle.color}; color: #000; padding: 15px 25px; border-radius: 10px; font-family: 'Orbitron'; font-weight: 900; text-align: center; box-shadow: ${cardStyle.glow};">
                            <div style="font-size: 0.6rem;">RATING</div>
                            <div style="font-size: 1.8rem;">${player.rat}</div>
                        </div>
                    </div>

                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                        ${this.renderStatCard("BMI (BODY-MASS)", bmi, (bmi > 25 ? "CHECK" : "OK"), (bmi > 25 ? "var(--status-error)" : "var(--neon-green)"))}
